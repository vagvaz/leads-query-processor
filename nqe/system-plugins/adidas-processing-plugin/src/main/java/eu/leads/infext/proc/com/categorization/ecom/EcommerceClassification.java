...
public class EcommerceClassification {
	...
	public boolean determinePageEcomFeatures() {
		...
		if(retValues != null && retValues.size() >= 4) {
			isEcomAssumption = (String) retValues.get(0);
			ecomFeatures = retValues.get(1).toString();
			buttonNode = retValues.get(2).toString().trim().length()>0 ? retValues.get(2).toString() : null; // might be None, is that ok?
			basketNode = retValues.get(3).toString().trim().length()>0 ? retValues.get(3).toString() : null; // the same
