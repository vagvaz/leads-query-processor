...
public class LQPConfiguration {
   ...
   * One can safely use the version with no parameters.*/
   ...
   private static void resolveDyanmicParameters() {
      instance.getConfiguration().setProperty("node.hostname", ConfigurationUtilities.resolveHostname());
   ...
   private static void loadSystemPropertiesFile() {
      try {
         ...
         if (config.containsKey("node.interface"))
            instance.getConfiguration().setProperty("node.ip", ConfigurationUtilities.resolveIp(config.getString("node.interface")));
   ...
   private static void updateConfigurationFiles() {
      try {
         ...
         String broadcast = instance.getConfiguration().getString("node.ip").substring(0, ip.lastIndexOf(".")) + ".255";
