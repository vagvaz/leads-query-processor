                ...
                if(node.getConfiguration().containsField("buildBloomFilter")){
                  ...
                  for(PlanNode s : sources){ // hijcak deployment and run 2nd run
                    if(s.getNodeId().equals(otherScanNode)){
                      next = s;
                      next.getConfiguration().getObject("next").getObject("configuration").putObject("useBloomFilter",node.getConfiguration().getObject("next").getObject("configuration").getObject("buildBloomFilter"));
  ...
  private void startExecution(ExecutionPlanMonitor executionPlan) {
    if (!executionPlan.shouldRunMapReduceFirst()) {
      ...
      if (sources.size() == 1) {
        ...
        bloomFilter.putString("bloomCache", executionPlan.getNode(node1.getOutput()).getOutput() + ".bloom");
        ...
        if (executionPlan.getNextOperator(node1).getNodeType().toString().equals(LeadsNodeType.JOIN.toString())) {
          if (hasPredicate(node1) && hasPredicate(node2)) {
            if(hasIndexedPredicate(node1) && hasIndexedPredicate(node2) || !(hasIndexedPredicate(node1) && hasIndexedPredicate(node2)) ){
              ...
              System.err.println("BOTH HAVE PRED BUT NOT INDEXED");
              if(node1Size < node2Size){
                System.err.println("CHOOSING NODE 2 " + node2.getNodeId());
                    //Choose node2
                    bloomFilter.putNumber("bloomSize", getSize(node2));
                node2.getConfiguration().getObject("next").getObject("configuration").putObject("buildBloomFilter",
                    bloomFilter);
                node2.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
                executionPlan.updateNode(node2);
                node1.getConfiguration().getObject("next").getObject("configuration").putObject("useBloomFilter",
                    bloomFilter);
                ...
                //choose node1
                System.err.println("CHOOSING NODE 1 " + node1.getNodeId());
                bloomFilter.putNumber("bloomSize",getSize(node1));
                node1.getConfiguration().getObject("next").getObject("configuration").putObject("buildBloomFilter",
                    bloomFilter);
                node1.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
                executionPlan.updateNode(node1);
                node2.getConfiguration().getObject("next").getObject("configuration").putObject("useBloomFilter",
                    bloomFilter);
                executionPlan.updateNode(node2);
              ...
              //choose node1
              System.err.println("CHOOSING INDEX NODE 1 " + node1.getNodeId());
              bloomFilter.putNumber("bloomSize",getSize(node1));
              node1.getConfiguration().getObject("next").getObject("configuration").putObject("buildBloomFilter",
                  bloomFilter);
              node1.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
              ...
              //choose node2
              System.err.println("CHOOSING INDEX NODE 2 " + node2.getNodeId());
              bloomFilter.putNumber("bloomSize", getSize(node2));
              node2.getConfiguration().getObject("next").getObject("configuration")
                  .putObject("buildBloomFilter", bloomFilter);
              node2.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
              executionPlan.updateNode(node2);
              node1.getConfiguration().getObject("next").getObject("configuration").putObject("useBloomFilter",
                  bloomFilter);
            ...
            //choose node 1
            System.err.println("CHOOSING SIMPLE NODE 1 " + node1.getNodeId());
            bloomFilter.putNumber("bloomSize",getSize(node1));
            node1.getConfiguration().getObject("next").getObject("configuration").putObject("buildBloomFilter",
                bloomFilter);
            node1.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
            ...
            //choose node2
            System.err.println("CHOOSING SIMPLE NODE 2 " + node2.getNodeId());
            bloomFilter.putNumber("bloomSize",getSize(node2));
            node2.getConfiguration().getObject("next").getObject("configuration").putObject("buildBloomFilter",
                bloomFilter);
            node2.getConfiguration().putString("buildBloomFilter", "buildBloomFilter");
            ...
            System.err.println("NOT CHOOOSING");
            for (PlanNode source : sources) {
              if (source != null) {
                deployOperator(executionPlan, source);
              }
