...
public class DeployerMonitorWorker extends Verticle implements LeadsMessageHandler {
   ...
   public void start() {
      ...
      periodicHandler = new PeriodicCheckHandler(id,deployerLogic,nqeGroup,log,com,actionToLevelMap,monitoredActions);
...
//         action.setStatus(ActionStatus.INPROCESS.toString());
         ...
         switch (ActionStatus.valueOf(action.getStatus())) {
            case PENDING: //probably received an action from an external source
               if (label.equals(DeployerConstants.OPERATOR_STARTED)) {
                  ...
                  log.error("Received Completed OPERATOR Action " + action.toString() + " with status PENDING" );
               }
               else if (label.equals(DeployerConstants.OPERATOR_OWNER)) {
                  log.error("Received OWNER OPERATOR Action " + action.toString() + " with status PENDING");
               }
               else if (label.equals(DeployerConstants.OPERATOR_RUNNING_STATUS)) {
                  log.error("Received RUNNING STATUS OPERATOR Action " + action.toString() + " with status PENDING");
               }
               else {
            ...
            case INPROCESS:
               if (label.equals(DeployerConstants.OPERATOR_STARTED)) {
                  log.error("Received OPERATOR STARTED Action " + action.toString() + " with status INPROCESS" );
                  return;
               }
               else if (label.equals(DeployerConstants.OPERATOR_COMPLETED)) {
                  log.error("Received Completed OPERATOR Action " + action.toString() + " with status INPROCESS" );
                  return;
               }
               else if (label.equals(DeployerConstants.OPERATOR_OWNER)) {
                  ...
                  if(claimedAction == null){
                     ...
                     log.error("Received OPERATOR OWNER but action " + action.toString() + " was not in unclaimed level but in level " + currentLevel.toString());
                  }
                  monitoredActions.get(1).put(action.getId(),action);
                  actionToLevelMap.put(action.getId(),1);
                  ...
                  if(currentLevel == null) {
                     log.error("Received OPERATOR STATUS but action " + action.toString() + " is not monitored " + currentLevel.toString());
                     return;
                  }
                  Action runningAction = monitoredActions.get(currentLevel).remove(action.getId());
            ...
            case COMPLETED: // the action either a part of a multistep workflow (INPROCESSING) or it could be processed.
               if (label.equals(DeployerConstants.OPERATOR_STARTED)) {
                  log.error("Received OPERATOR STARTED Action " + action.toString() + " with status COMPLETED");
                  return;
               }
               else if (label.equals(DeployerConstants.OPERATOR_COMPLETED)) {
                  ...
                  log.error("Received OPERATOR OWNER but action " + action.toString() + " with status COMPLETED");
                  return;
               }
               else if (label.equals(DeployerConstants.OPERATOR_RUNNING_STATUS)) {
                  log.error("Received OPERATOR STATUS but action " + action.toString() + " with status COMPLETED");
                  return;
               }
               else {
                  log.error("Unknown INPROCESS Action received " + action.toString());
   ...
   private void completeOperator(Action action) {
      ...
      if(completedAction == null){
         log.error("Action " + action.toString() + " was not in level " + currentLevel.toString() + " of DeployerMonitor" + id);
