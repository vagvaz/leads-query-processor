...
public class FileHandlerOutput<K extends Serializable, V> implements OutputHandler<K, V> {
    ...
    @Override public void initialize(Properties conf) {
        ...
        try {
            ...
            if (storeNutchData) {
                ...
                dataWriter = new ObjectOutputStream(new FileOutputStream(valueFile));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void rollFile() {
        ...
        try {
            ...
            keyWriter.close();
            keyWriter =
                new ObjectOutputStream(new FileOutputStream(baseDir + "/" + filename + "." + fileCounter + ".keys"));
            if (storeNutchData) {
                valueWriter.close();
                valueWriter.create(WebPage.SCHEMA$, new File(baseDir + "/" + filename + "." + fileCounter + ".values"));
            } else {
                dataWriter.close();
                dataWriter = new ObjectOutputStream(
                    new FileOutputStream(baseDir + "/" + filename + "." + fileCounter + ".values"));
            }
