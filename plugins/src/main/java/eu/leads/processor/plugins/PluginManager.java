...
public class PluginManager {
   ...
   private static boolean validatePlugin(PluginPackage plugin) {
      ...
      if (!checkPluginName(plugin.getClassName(), plugin.getJar())) {
         log.error("Plugin " + plugin.getId() + " failed validation test. Class.getClassByName(" + plugin.getClassName() + ")");
         return false;
      }
   ...
   private static void upload(String pluginCache, PluginPackage plugin) {
      Cache cache = (Cache) InfinispanClusterSingleton.getInstance().getManager().getPersisentCache(pluginCache);
   ...
   private static boolean checkInstantiate(String className, byte[] jar, byte[] config) {
      boolean result = true;
      String name = System.getProperty("java.io.tmpdir") + "/leads/processor/tmp/testplugin.jar";
      String configName = System.getProperty("java.io.tmpdir") + "/leads/processor/tmp/testplugin.xml";
   ...
   public static boolean deployPlugin(String pluginId, String cacheName, EventType[] events) {
      Cache configCache = (Cache) InfinispanClusterSingleton.getInstance().getManager().getPersisentCache(StringConstants.PLUGIN_ACTIVE_CACHE);
      Cache pluginsCache = (Cache) InfinispanClusterSingleton.getInstance().getManager().getPersisentCache(StringConstants.PLUGIN_CACHE);
      boolean result = true;
      ...
      if (!validatePlugin(plugin)) {
         log.warn("Plugin " + plugin.getClassName() + " could not be validated so it could not be deployed");
         result = false;
      }
      int eventmask = 0;
      ...
      deployPluginListener(plugin.getId(), cacheName, InfinispanClusterSingleton.getInstance().getManager());
      return result;
   }
   ...
   private static void deployPluginListener(String pluginId, String cacheName, InfinispanManager manager) {
   ...
   public static boolean deployPlugin(String pluginId, XMLConfiguration config, String cacheName, EventType[] events) {
      Cache configCache = (Cache) InfinispanClusterSingleton.getInstance().getManager().getPersisentCache(StringConstants.PLUGIN_ACTIVE_CACHE);
      Cache pluginsCache = (Cache) InfinispanClusterSingleton.getInstance().getManager().getPersisentCache(StringConstants.PLUGIN_CACHE);
      boolean result = true;
      PluginPackage plugin = (PluginPackage) pluginsCache.get(pluginId);
...
//            des.submitEverywhere(new AddListenerCallable(cache.getName(),listener));
   ...
   public static void deployLocalPlugin(PluginInterface plugin, XMLConfiguration config, String cacheName, EventType[] events, InfinispanManager manager) {
      Properties conf = new Properties();
      conf.put("target", cacheName);
      conf.put("config", StringConstants.PLUGIN_ACTIVE_CACHE);
      LinkedList<String> alist = new LinkedList<String>();
