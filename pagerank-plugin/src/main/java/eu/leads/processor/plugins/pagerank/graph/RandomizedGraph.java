...
public abstract class RandomizedGraph {
    ...
    public RandomizedGraph(int R, Configuration configuration, InfinispanManager infinispanManager,
                              int seed) {
    ...
    private void initCacheAndAttrs(Configuration configuration,
                                      InfinispanManager infinispanManager) {
        ...
        vc_per_node =
            (Cache) infinispanManager.getPersisentCache(configuration.getString("vc_cache"));
    ...
    public void sendVisitDriftIfNeeded(int drift) {
        ...
        if (vc_per_node.containsKey(nodeName)) {
            ...
            if (Math.abs(accurateLocalSum - old_estimate) >
                ...
                vc_per_node.put(Const.GLOBAL_SUM,
                                   (Integer) vc_per_node.get(Const.GLOBAL_SUM) + (accurateLocalSum
                                                                                      - old_estimate));
            }
        } else {
            vc_per_node.put(nodeName, accurateLocalSum);
