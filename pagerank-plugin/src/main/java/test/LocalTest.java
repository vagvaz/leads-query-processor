...
public class LocalTest {
    ...
    public static void main(String[] args) {
        ...
        Map cache = InfinispanClusterSingleton.getInstance().getManager()
    ...
    private static void printVCs(Map mymap) {
        ...
        int global_vc = (Integer) InfinispanClusterSingleton.getInstance().getManager()
                                      .getPersisentCache("vc_per_node").get(Const.GLOBAL_SUM);
        Iterator<Map.Entry> my_iterator = mymap.entrySet().iterator();

        while (my_iterator.hasNext()) {

            Map.Entry my_entry = my_iterator.next();
            System.out.println(my_entry.getKey() + "->"
                                   + (double) ((DSPMNode) my_entry.getValue()).getVisitCount()
                                         / (double) global_vc);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // retrieval of visit count of each cache's value, by calling : webpage.getVisitCount();
        Map cache1 = InfinispanClusterSingleton.getInstance().getManager()
                         .getPersisentCache("pagerankCache");
        Map cache2 = InfinispanClusterSingleton.getInstance().getManager()
    ...
    private static void printVCs(Map cache1, Map cache2) {
        ...
        System.out.println("Accurate global sum: " + realSum + ", e-approximate global sum: "
                               + w.getLatestReceivedGlobalSum());
    }
}
