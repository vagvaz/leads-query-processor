...
import static grammar.LeadsSQLParser.*;
...
//import static org.apache.tajo.engine.parser.LeadsSQLParser.*;
...
public class LeadsSQLAnalyzer extends LeadsSQLParserBaseVisitor<Expr> {
    ...
    public static OpType tokenToExprType(int tokenId) {
        switch (tokenId) {
            case LeadsSQLParser.UNION:
                return OpType.Union;
            case LeadsSQLParser.EXCEPT:
                return OpType.Except;
            case LeadsSQLParser.INTERSECT:
                return OpType.Intersect;

            case LeadsSQLParser.AND:
                return OpType.And;
            case LeadsSQLParser.OR:
                return OpType.Or;

            case LeadsSQLParser.EQUAL:
                return OpType.Equals;
            case LeadsSQLParser.NOT_EQUAL:
                return OpType.NotEquals;
            case LeadsSQLParser.LTH:
                return OpType.LessThan;
            case LeadsSQLParser.LEQ:
                return OpType.LessThanOrEquals;
            case LeadsSQLParser.GTH:
                return OpType.GreaterThan;
            case LeadsSQLParser.GEQ:
                return OpType.GreaterThanOrEquals;

            case LeadsSQLParser.MULTIPLY:
                return OpType.Multiply;
            case LeadsSQLParser.DIVIDE:
                return OpType.Divide;
            case LeadsSQLParser.MODULAR:
                return OpType.Modular;
            case LeadsSQLParser.PLUS:
                return OpType.Plus;
            case LeadsSQLParser.MINUS:
                return OpType.Minus;
    ...
    public Expr parse(String sql) {
        ...
        this.parser = new LeadsSQLParser(tokens);
    ...
    @Override
    public Expr visitNon_join_query_expression(LeadsSQLParser.Non_join_query_expressionContext ctx) {
        ...
        for (int i = 1; i < ctx.getChildCount(); i++) {
            ...
            if (ctx.getChild(idx) instanceof TerminalNode) {
                ...
                LeadsSQLParser.Query_termContext queryTermContext =
                    (LeadsSQLParser.Query_termContext) ctx.getChild(idx);
    ...
    @Override
    public Expr visitQuery_specification(LeadsSQLParser.Query_specificationContext ctx) {
        Expr current = null;
     ...
     */
    @Override
    public Projection visitSelect_list(LeadsSQLParser.Select_listContext ctx) {
     ...
     */
    @Override
    public NamedExpr visitSelect_sublist(LeadsSQLParser.Select_sublistContext ctx) {
    ...
    @Override
    public Selection visitWhere_clause(LeadsSQLParser.Where_clauseContext ctx) {
        return new Selection(visitSearch_condition(ctx.search_condition()));
    }

    @Override
    public Aggregation visitGroupby_clause(LeadsSQLParser.Groupby_clauseContext ctx) {
        ...
        if (ctx.grouping_element_list().grouping_element().get(0).empty_grouping_set() == null) {
            ...
            for (int i = 0; i < elementSize; i++) {
                LeadsSQLParser.Grouping_elementContext element =
                ...
                if (element.ordinary_grouping_set() != null) {
                    ...
                    Collections.addAll(ordinaryExprs,
                                          getRowValuePredicandsFromOrdinaryGroupingSet(element
                                                                                           .ordinary_grouping_set()));
                } else if (element.rollup_list() != null) {
                    groupSize++;
                    groups.add(new GroupElement(GroupType.Rollup,
                                                   getRowValuePredicandsFromOrdinaryGroupingSetList(element
                                                                                                        .rollup_list().c)));
            ...
            if (ordinaryExprs != null) {
                groups.set(0, new GroupElement(GroupType.OrdinaryGroup, ordinaryExprs
                                                                            .toArray(new Expr[ordinaryExprs
                                                                                                  .size()])));
                clause.setGroups(groups.subList(0, groupSize).toArray(new GroupElement[groupSize]));
            } else if (groupSize > 1) {
                clause.setGroups(groups.subList(1, groupSize)
                                     .toArray(new GroupElement[groupSize - 1]));
            }
        }
    ...
    @Override public WindowFunctionExpr visitWindow_function(@NotNull
    LeadsSQLParser.Window_functionContext context) {
        ...
        Window_name_or_specificationContext windowNameOrSpec =
            context.window_name_or_specification();
    ...
    @Override
    public Window visitWindow_clause(@NotNull LeadsSQLParser.Window_clauseContext ctx) {
        Window.WindowDefinition[] definitions =
            new Window.WindowDefinition[ctx.window_definition_list().window_definition().size()];
        for (int i = 0; i < definitions.length; i++) {
            Window_definitionContext windowDefinitionContext =
                ctx.window_definition_list().window_definition(i);
    ...
    public WindowSpec buildWindowSpec(LeadsSQLParser.Window_specificationContext ctx) {
        WindowSpec windowSpec = new WindowSpec();
        if (checkIfExist(ctx.window_specification_details())) {
            Window_specification_detailsContext windowSpecDetail =
                ctx.window_specification_details();
            ...
            if (checkIfExist(windowSpecDetail.window_order_clause())) {
                windowSpec.setSortSpecs(
                                           buildSortSpecs(windowSpecDetail.window_order_clause()
                                                              .orderby_clause()
                                                              .sort_specifier_list()));
            }
            ...
            if (checkIfExist(windowSpecDetail.window_frame_clause())) {
                ...
                if (checkIfExist(frameContext.window_frame_extent()
                                     .window_frame_between())) { // when 'between' is given
                    Window_frame_betweenContext between =
                        frameContext.window_frame_extent().window_frame_between();
                    WindowSpec.WindowStartBound startBound =
                        buildWindowStartBound(between.window_frame_start_bound());
                    WindowSpec.WindowEndBound endBound =
                        buildWindowEndBound(between.window_frame_end_bound());

                    windowFrame = new WindowSpec.WindowFrame(unit, startBound, endBound);
                } else { // if there is only start bound
                    WindowSpec.WindowStartBound startBound =
                        buildWindowStartBound(frameContext.window_frame_extent()
    ...
    public WindowSpec.WindowStartBound buildWindowStartBound(Window_frame_start_boundContext context) {
        ...
        if (boundType == WindowFrameStartBoundType.PRECEDING) {
            bound.setNumber(visitUnsigned_value_specification(context
    ...
    public WindowSpec.WindowEndBound buildWindowEndBound(Window_frame_end_boundContext context) {
        ...
        if (boundType == WindowFrameEndBoundType.FOLLOWING) {
            endBound.setNumber(visitUnsigned_value_specification(context
                                                                     .unsigned_value_specification()));
        }
    ...
    public Sort.SortSpec[] buildSortSpecs(Sort_specifier_listContext context) {
        ...
        for (int i = 0; i < size; i++) {
            LeadsSQLParser.Sort_specifierContext specContext = context.sort_specifier(i);
    ...
    @Override
    public Sort visitOrderby_clause(LeadsSQLParser.Orderby_clauseContext ctx) {
        return new Sort(buildSortSpecs(ctx.sort_specifier_list()));
    }

    @Override
    public Limit visitLimit_clause(LeadsSQLParser.Limit_clauseContext ctx) {
        return new Limit(visitNumeric_value_expression(ctx.numeric_value_expression()));
    }

    @Override
    public Expr visitJoined_table(LeadsSQLParser.Joined_tableContext ctx) {
    ...
    @Override
    public Join visitJoined_table_primary(LeadsSQLParser.Joined_table_primaryContext ctx) {
        Join join;
        if (ctx.CROSS() != null) {
            ...
            if (ctx.join_type() != null && ctx.join_type().outer_join_type() != null) {
                Outer_join_type_part2Context outer_join_typeContext =
                    ctx.join_type().outer_join_type()
            ...
            if (ctx.join_specification() != null) { // only for qualified join
                if (ctx.join_specification().join_condition() != null) {
                    ...
                    ColumnReferenceExpr[] columns =
                        buildColumnReferenceList(ctx.join_specification().
                                                                             named_columns_join()
    ...
    private Expr[] getRowValuePredicandsFromOrdinaryGroupingSet(Ordinary_grouping_setContext ctx) {
        ...
        if (ctx.row_value_predicand_list() != null) {
            Collections.addAll(rowValuePredicands,
                                  buildRowValuePredicands(ctx.row_value_predicand_list()));
        }
        return rowValuePredicands.toArray(new Expr[rowValuePredicands.size()]);
    }
    ...
    public Expr visitTable_primary(LeadsSQLParser.Table_primaryContext ctx) {
        if (ctx.table_or_query_name() != null) {
            ...
            return new TablePrimarySubQuery(ctx.name.getText(),
    ...
    @Override
    public Expr visitSubquery(LeadsSQLParser.SubqueryContext ctx) {
        return visitQuery_expression(ctx.query_expression());
    }

    @Override
    public BetweenPredicate visitBetween_predicate(LeadsSQLParser.Between_predicateContext ctx) {
        ...
        return new BetweenPredicate(checkIfExist(ctx.between_predicate_part_2().NOT()),
                                       checkIfExist(ctx.between_predicate_part_2().SYMMETRIC()),
                                       predicand, begin, end);
    }

    @Override
    public CaseWhenPredicate visitSimple_case(LeadsSQLParser.Simple_caseContext ctx) {
        ...
        for (int i = 0; i < ctx.simple_when_clause().size(); i++) {
            ...
            BinaryOperator bin = new BinaryOperator(OpType.Equals, leftTerm,
                                                       visitValue_expression(simpleWhenCtx
                                                                                 .search_condition()
                                                                                 .value_expression()));
    ...
    @Override
    public CaseWhenPredicate visitSearched_case(LeadsSQLParser.Searched_caseContext ctx) {
        CaseWhenPredicate caseWhen = new CaseWhenPredicate();
    ...
    @Override
    public Expr visitCommon_value_expression(LeadsSQLParser.Common_value_expressionContext ctx) {
    ...
    @Override
    public Expr visitParenthesized_value_expression(LeadsSQLParser.Parenthesized_value_expressionContext ctx) {
        return visitValue_expression(ctx.value_expression());
    }

    @Override
    public Expr visitBoolean_value_expression(LeadsSQLParser.Boolean_value_expressionContext ctx) {
        Expr current = visitOr_predicate(ctx.or_predicate());
        return current;
    }

    @Override
    public Expr visitOr_predicate(LeadsSQLParser.Or_predicateContext ctx) {
    ...
    @Override
    public Expr visitAnd_predicate(LeadsSQLParser.And_predicateContext ctx) {
    ...
    @Override
    public Expr visitBoolean_factor(LeadsSQLParser.Boolean_factorContext ctx) {
        if (ctx.NOT() != null) {
    ...
    @Override
    public Expr visitBoolean_test(LeadsSQLParser.Boolean_testContext ctx) {
    ...
    @Override
    public Expr visitBoolean_primary(LeadsSQLParser.Boolean_primaryContext ctx) {
    ...
    @Override
    public Expr visitBoolean_predicand(LeadsSQLParser.Boolean_predicandContext ctx) {
        if (checkIfExist(ctx.nonparenthesized_value_expression_primary())) {
            ...
            return visitBoolean_value_expression(ctx.parenthesized_boolean_value_expression()
                                                     .boolean_value_expression());
        }
    }
    ...
    public Expr visitNonparenthesized_value_expression_primary(
                                                                  LeadsSQLParser.Nonparenthesized_value_expression_primaryContext ctx) {
        return visitChildren(ctx);
    }

    @Override
    public Expr visitRow_value_predicand(LeadsSQLParser.Row_value_predicandContext ctx) {
    ...
    @Override
    public Expr visitRow_value_constructor_predicand(LeadsSQLParser.Row_value_constructor_predicandContext ctx) {
    ...
    @Override
    public BinaryOperator visitComparison_predicate(LeadsSQLParser.Comparison_predicateContext ctx) {
    ...
    @Override
    public Expr visitNumeric_value_expression(LeadsSQLParser.Numeric_value_expressionContext ctx) {
    ...
    @Override
    public Expr visitTerm(LeadsSQLParser.TermContext ctx) {
    ...
    @Override
    public Expr visitFactor(LeadsSQLParser.FactorContext ctx) {
    ...
    @Override
    public Expr visitNumeric_primary(LeadsSQLParser.Numeric_primaryContext ctx) {
        Expr current = null;
    ...
    @Override
    public InPredicate visitIn_predicate(LeadsSQLParser.In_predicateContext ctx) {
        return new InPredicate(visitChildren(ctx.numeric_value_expression()),
                                  visitIn_predicate_value(ctx.in_predicate_value()),
                                  ctx.NOT() != null);
    }

    @Override
    public Expr visitIn_predicate_value(LeadsSQLParser.In_predicate_valueContext ctx) {
    ...
    @Override
    public Expr visitArray(LeadsSQLParser.ArrayContext ctx) {
    ...
    @Override
    public Expr visitPattern_matching_predicate(LeadsSQLParser.Pattern_matching_predicateContext ctx) {
        ...
        if (checkIfExist(ctx.pattern_matcher().negativable_matcher())) {
            ...
            if (checkIfExist(matcher.LIKE())) {
                ...
                return new PatternMatchPredicate(OpType.SimilarToPredicate, not, predicand,
                                                    pattern);
    ...
    @Override
    public IsNullPredicate visitNull_predicate(LeadsSQLParser.Null_predicateContext ctx) {
        Expr predicand = visitRow_value_predicand(ctx.row_value_predicand());
        return new IsNullPredicate(ctx.NOT() != null, predicand);
    }

    @Override
    public ExistsPredicate visitExists_predicate(LeadsSQLParser.Exists_predicateContext ctx) {
        return new ExistsPredicate(new SimpleTableSubQuery(visitTable_subquery(ctx.table_subquery())),
                                      ctx.NOT() != null);
    }

    @Override
    public ColumnReferenceExpr visitColumn_reference(LeadsSQLParser.Column_referenceContext ctx) {
        ColumnReferenceExpr column = new ColumnReferenceExpr(ctx.name.getText());
        if (checkIfExist(ctx.db_name)) {
            column.setQualifier(CatalogUtil
                                    .buildFQName(ctx.db_name.getText(), ctx.tb_name.getText()));
    ...
    @Override
    public LiteralValue visitUnsigned_numeric_literal(@NotNull
    LeadsSQLParser.Unsigned_numeric_literalContext ctx) {
        if (ctx.NUMBER() != null) {
    ...
    @Override
    public GeneralSetFunctionExpr visitAggregate_function(LeadsSQLParser.Aggregate_functionContext ctx) {
    ...
    @Override
    public GeneralSetFunctionExpr visitGeneral_set_function(LeadsSQLParser.General_set_functionContext ctx) {
        String signature = ctx.set_function_type().getText();
        boolean distinct =
            checkIfExist(ctx.set_qualifier()) && checkIfExist(ctx.set_qualifier().DISTINCT());
    ...
    @Override
    public FunctionExpr visitRoutine_invocation(LeadsSQLParser.Routine_invocationContext ctx) {
    ...
    @Override
    public Expr visitCharacter_string_type(LeadsSQLParser.Character_string_typeContext ctx) {
        return new LiteralValue(stripQuote(ctx.getText()), LiteralType.String);
    }

    @Override
    public Expr visitCharacter_value_expression(LeadsSQLParser.Character_value_expressionContext ctx) {
        ...
        for (int i = 1; i < ctx.getChildCount(); i++) {
            ...
            if (left.getType() == OpType.Literal && right.getType() == OpType.Literal) {
                current = new LiteralValue(((LiteralValue) left).getValue() + ((LiteralValue) right)
                                                                                  .getValue(),
                                              LiteralType.String);
            } else {
    ...
    public Expr visitExtract_expression(Extract_expressionContext ctx) {
        Expr extractTarget =
            new LiteralValue(ctx.extract_field_string.getText(), LiteralType.String);
        Expr extractSource =
    ...
    @Override
    public Expr visitTrim_function(LeadsSQLParser.Trim_functionContext ctx) {
    ...
    @Override
    public Expr visitDatabase_definition(@NotNull LeadsSQLParser.Database_definitionContext ctx) {
        return new CreateDatabase(ctx.identifier().getText(), null,
                                     checkIfExist(ctx.if_not_exists()));
    }

    @Override
    public Expr visitDrop_database_statement(@NotNull
    LeadsSQLParser.Drop_database_statementContext ctx) {
        return new DropDatabase(ctx.identifier().getText(), checkIfExist(ctx.if_exists()));
    }

    @Override
    public Expr visitCreate_table_statement(LeadsSQLParser.Create_table_statementContext ctx) {
    ...
    LeadsSQLParser.Truncate_table_statementContext ctx) {
    ...
    private ColumnDefinition[] getDefinitions(LeadsSQLParser.Table_elementsContext ctx) {
    ...
    public PartitionMethodDescExpr parseTablePartitioningClause(LeadsSQLParser.Table_partitioning_clausesContext ctx) {
        ...
        if (checkIfExist(ctx.range_partitions())) { // For Range Partition
            ...
            for (Range_value_clauseContext rangeValue : rangeValueClause) {
                if (checkIfExist(rangeValue.MAXVALUE())) { // LESS THAN (MAXVALUE)
                    specifiers
                        .add(new RangePartitionSpecifier(rangeValue.partition_name().getText()));
                } else { // LESS THAN (expr)
                    specifiers
                        .add(new RangePartitionSpecifier(rangeValue.partition_name().getText(),
                                                            visitValue_expression(rangeValue
                                                                                      .value_expression())));
                }
            }
            return new CreateTable.RangePartition(buildColumnReferenceList(ctx.range_partitions()
                                                                               .column_reference_list()),
                                                     specifiers);
            ...
            if (checkIfExist(hashPartitions.hash_partitions_by_quantity())) { // PARTITIONS (num)
                return new HashPartition(buildColumnReferenceList(hashPartitions
                                                                      .column_reference_list()),
                                            visitNumeric_value_expression(hashPartitions
                                                                              .hash_partitions_by_quantity().quantity));
                ...
                for (Individual_hash_partitionContext partition :
                    ...
                    specifiers.add(new CreateTable.PartitionSpecifier(partition.partition_name()
                                                                          .getText()));
                }
                return new HashPartition(buildColumnReferenceList(hashPartitions
                                                                      .column_reference_list()),
            ...
            List<List_value_partitionContext> partitions =
                listPartitions.list_value_clause_list().list_value_partition();
            ...
            for (List_value_partitionContext listValuePartition : partitions) {
                ...
                Expr[] exprs = new Expr[size];
                for (int i = 0; i < size; i++) {
                    exprs[i] = visitRow_value_predicand(listValuePartition.in_value_list()
                                                            .row_value_predicand(i));
                }
                specifiers
                    .add(new ListPartitionSpecifier(listValuePartition.partition_name().getText(),
                                                       new ValueListExpr(exprs)));
            }
            return new ListPartition(buildColumnReferenceList(ctx.list_partitions()
                                                                  .column_reference_list()),
                                        specifiers);
    ...
    @Override
    public DataTypeExpr visitData_type(LeadsSQLParser.Data_typeContext ctx) {
        LeadsSQLParser.Predefined_typeContext predefined_type = ctx.predefined_type();

        DataTypeExpr typeDefinition = null;
        if (predefined_type.character_string_type() != null) {
            LeadsSQLParser.Character_string_typeContext character_string_type =
            ...
            if ((character_string_type.CHARACTER() != null || character_string_type.CHAR() != null)
                ...
                if (character_string_type.type_length() != null) {
                    typeDefinition.setLengthOrPrecision(
                                                           Integer.parseInt(character_string_type
                                                                                .type_length()
                                                                                .NUMBER()
                                                                                .getText()));
                }
            ...
            LeadsSQLParser.National_character_string_typeContext nchar_type =
            ...
            if (nchar_type.type_length() != null) {
                typeDefinition.setLengthOrPrecision(
                                                       Integer.parseInt(nchar_type.type_length()
                                                                            .NUMBER().getText()));
            }

        } else if (predefined_type.binary_large_object_string_type() != null) {
            LeadsSQLParser.Binary_large_object_string_typeContext blob_type =
            ...
            if (predefined_type.numeric_type().exact_numeric_type() != null) {
                    ...
                    if (exactType.precision_param() != null) {
                        if (exactType.precision_param().scale != null) {
                            typeDefinition.setScale(
                                                       Integer.parseInt(exactType
                                                                            .precision_param().scale
                                                                            .getText()));
                        }
                        typeDefinition.setLengthOrPrecision(
                                                               Integer.parseInt(exactType
                ...
                LeadsSQLParser.Approximate_numeric_typeContext approximateType =
            ...
            LeadsSQLParser.Datetime_typeContext dateTimeType = predefined_type.datetime_type();
            ...
            LeadsSQLParser.Bit_typeContext bitType = predefined_type.bit_type();
            ...
            if (bitType.type_length() != null) {
                typeDefinition.setLengthOrPrecision(
                                                       Integer
                                                           .parseInt(bitType.type_length().NUMBER()
                                                                         .getText()));
            }
        } else if (predefined_type.binary_type() != null) {
            LeadsSQLParser.Binary_typeContext binaryType = predefined_type.binary_type();
            ...
            if (binaryType.type_length() != null) {
                typeDefinition.setLengthOrPrecision(
                                                       Integer.parseInt(binaryType.type_length()
                                                                            .NUMBER().getText()));
            }
    ...
    @Override
    public Expr visitInsert_statement(LeadsSQLParser.Insert_statementContext ctx) {
    ...
    @Override
    public Expr visitDrop_table_statement(LeadsSQLParser.Drop_table_statementContext ctx) {
        return new DropTable(ctx.table_name().getText(), checkIfExist(ctx.if_exists()),
                                checkIfExist(ctx.PURGE()));
    }

    private Map<String, String> getParams(LeadsSQLParser.Param_clauseContext ctx) {
    ...
    @Override
    public Expr visitCast_specification(LeadsSQLParser.Cast_specificationContext ctx) {
    ...
    @Override
    public Expr visitUnsigned_value_specification(@NotNull
    LeadsSQLParser.Unsigned_value_specificationContext ctx) {
        return visitChildren(ctx);
    }

    @Override
    public Expr visitUnsigned_literal(@NotNull LeadsSQLParser.Unsigned_literalContext ctx) {
    ...
    @Override
    public Expr visitGeneral_literal(LeadsSQLParser.General_literalContext ctx) {
        if (checkIfExist(ctx.Character_String_Literal())) {
            return new LiteralValue(stripQuote(ctx.Character_String_Literal().getText()),
                                       LiteralType.String);
    ...
    @Override
    public Expr visitDatetime_literal(@NotNull LeadsSQLParser.Datetime_literalContext ctx) {
    ...
    @Override
    public Expr visitTime_literal(LeadsSQLParser.Time_literalContext ctx) {
        String timePart = stripQuote(ctx.time_string.getText());
        return new TimeLiteral(parseTime(timePart));
    }

    @Override
    public Expr visitDate_literal(LeadsSQLParser.Date_literalContext ctx) {
        String datePart = stripQuote(ctx.date_string.getText());
        return new DateLiteral(parseDate(datePart));
    }

    @Override
    public Expr visitTimestamp_literal(LeadsSQLParser.Timestamp_literalContext ctx) {
    ...
    @Override
    public Expr visitInterval_literal(@NotNull LeadsSQLParser.Interval_literalContext ctx) {
        String intervalStr = stripQuote(ctx.interval_string.getText());
        return new IntervalLiteral(intervalStr);
    }

    @Override
    public Expr visitDatetime_value_expression(@NotNull
    LeadsSQLParser.Datetime_value_expressionContext ctx) {
        return visitDatetime_term(ctx.datetime_term());
    }

    @Override
    public Expr visitDatetime_term(@NotNull LeadsSQLParser.Datetime_termContext ctx) {
        return visitDatetime_factor(ctx.datetime_factor());
    }

    @Override
    public Expr visitDatetime_factor(@NotNull LeadsSQLParser.Datetime_factorContext ctx) {
        return visitDatetime_primary(ctx.datetime_primary());
    }

    @Override
    public Expr visitDatetime_primary(@NotNull LeadsSQLParser.Datetime_primaryContext ctx) {
        if (checkIfExist(ctx.value_expression_primary())) {
    ...
    @Override
    public Expr visitDatetime_value_function(@NotNull
    LeadsSQLParser.Datetime_value_functionContext ctx) {
    ...
    @Override
    public Expr visitCurrent_date_value_function(@NotNull
    LeadsSQLParser.Current_date_value_functionContext ctx) {
    ...
    @Override
    public Expr visitCurrent_time_value_function(@NotNull
    LeadsSQLParser.Current_time_value_functionContext ctx) {
    ...
    @Override
    public Expr visitCurrent_timestamp_value_function(@NotNull
    LeadsSQLParser.Current_timestamp_value_functionContext ctx) {
    ...
    @Override
    public Expr visitAlter_tablespace_statement(@NotNull
    LeadsSQLParser.Alter_tablespace_statementContext ctx) {
    ...
    @Override
    public Expr visitAlter_table_statement(LeadsSQLParser.Alter_table_statementContext ctx) {
        ...
        if (checkIfExist(field_elementContext)) {
            ...
            final DataTypeExpr typeDef =
                visitData_type(field_elementContext.field_type().data_type());
    ...
    private AlterTableOpType determineAlterTableType(LeadsSQLParser.Alter_table_statementContext ctx) {
