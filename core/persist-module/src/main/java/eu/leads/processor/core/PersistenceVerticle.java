...
//         vertx.eventBus().registerHandler(id,dispatcher);
   ...
   public  JsonObject readAction(JsonObject msg) {
      ...
      if (!Strings.isNullOrEmpty(jsonValue)) {
         ...
         result.putString("message","Key " + msg.getString("key") + " does not exist in component cache " );
...
//      }
   ...
   public JsonObject getBactchAction(JsonObject msg){
      ...
      Cache cache = getCache(cacheName);
      try {
         CloseableIterable<Map.Entry<String, String>> iterable = cache.getAdvancedCache().filterEntries(new RangeFilter(min, max));
         ...
         logUtil.error("Iterating over " + cacheName + " for batch resulted in Exception " + e.getMessage() + "\n from msg " + msg.toString());
   ...
   public JsonObject getContainsAction(JsonObject msg) {
      ...
      try {
         ...
         logUtil.error("Checking for contains in" + cacheName + " for" +  key + "  resulted in Exception " + e.getMessage() + "\n from msg " + msg.toString());
         result.putString("status","failed");
