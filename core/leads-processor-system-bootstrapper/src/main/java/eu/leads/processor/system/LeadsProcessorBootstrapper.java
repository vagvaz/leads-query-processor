...
public class LeadsProcessorBootstrapper {
   ...
   static Logger logger = LoggerFactory.getLogger(LeadsProcessorBootstrapper.class.getCanonicalName());
   public static void main(String[] args) {
      if(checkArguments(args)){
         ...
         components = LQPConfiguration.getInstance().getConf().getStringArray("processor.components");
         configurationFiles = LQPConfiguration.getInstance().getConf().getStringArray("processor.configurationFiles");
      }
...
//         p.waitFor();
   ...
   private static void sendConfigurationTo(JsonObject config, String ip) {
      ...
      String command = "scp " + tmpFile +" "+   LQPConfiguration.getInstance().getConf().getString("processor.ssh.username")+"@" + ip + ":"+tmpFile;
      try {
   ...
   private static JsonObject generateConfiguration(String component) {
      ...
      result.putString("processors", LQPConfiguration.getConf().getString("processor.processors"));
      ...
      if(component.equals("imanager")){
        ...
        JsonObject logic = new JsonObject();
         ...
         logic.putString("publish",LQPConfiguration.getConf().getString("components.default.publish"));
         logic.putString("start",LQPConfiguration.getConf().getString("components.default.start"));
         result.putObject("logic",logic);
      }
      else{
         logger.error("Error Cannot Create configuration because unknown component " + component) ;
      }
      return result;
   }
   ...
   private static JsonObject getServiceConfiguration(String service) throws Exception {
      ...
      if(service.equals("planner-catalog-service")){
         result.putString("ip",LQPConfiguration.getInstance().getConf().getString("planner.catalog.ip"));
         result.putNumber("port",LQPConfiguration.getInstance().getConf().getInt("planner.catalog.port"));
      }
