...
//  private int batchSize = ;
  ...
  public MapDBIndex(String baseDir, String name){
    ...
    try {
      theDb = DBMaker.tempFileDB().transactionDisable().closeOnJvmShutdown().deleteFilesAfterClose().asyncWriteEnable().asyncWriteQueueSize(
          50000).executorEnable().make();
      keysDB = theDb.createTreeMap(keydbFile.getName()).nodeSize(1024).make(); // /dbfactory.open(keydbFile,options.verifyChecksums(true));
