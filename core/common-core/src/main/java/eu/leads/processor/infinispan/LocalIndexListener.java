...
//    }
    ...
    @Override public void initialize(InfinispanManager manager,JsonObject conf) {
        ...
        for(int i =0; i < parallelism; i++) {
            indexes.add(new LevelDBIndex( System.getProperties().getProperty("java.io.tmpdir")+"/"+StringConstants.TMPPREFIX+"/interm-index/"+ InfinispanClusterSingleton.getInstance().getManager().getUniquePath()+"/"+cacheName+i,cacheName+".index-"+i));
        }
