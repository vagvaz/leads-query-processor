...
public class LeadsComponentHandler implements LeadsMessageHandler {
   ...
   public LeadsComponentHandler(Component owner, Node com, LogProxy log, PersistenceProxy persistence) {
      this.owner = owner;
      this.com = com;
      this.log = log;
      ...
      if (message.getString("type").equals("command")) {
         ...
         switch (command) {
            ...
            case RESET:
               ...
               break;
            case UNKNOWN:
               log.error(owner.getComponentType() + ":" + owner.getId() + " received an unknown command \n" + message.toString());
               break;
         }
      }
   }
