...
public class GroupByNewNode extends DataTransNode implements GetInputs {
		...
		try {
			...
			while (rs.next()) {
				try {
					...
					TableInfo ti = getTiForConnection(GroupByNewNode.OUTPUT_CONN_POINT);
	...
	public List<Record> getInputs() {
		TableInfo ti = getTiForConnection(GroupByNewNode.INPUT_CONN_POINT);
		if (ti == null) {
			return null;
		}
