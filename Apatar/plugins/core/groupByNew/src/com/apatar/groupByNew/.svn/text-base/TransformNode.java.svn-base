...
public class TransformNode extends DataTransNode implements GetInputs {
	...
	public ImageIcon getIcon() {
		return TransformUtils.TRANSFORM_ICON;
	}
		...
		try {
			...
			while (rs.next()) {
				try {
					...
					TableInfo ti = getTiForConnection(TransformNode.OUTPUT_CONN_POINT);
	...
	public List<Record> getInputs() {
		TableInfo ti = getTiForConnection(TransformNode.INPUT_CONN_POINT);
		if (ti == null) {
			return null;
		}
