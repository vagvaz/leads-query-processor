...
public class ProjectNode extends DataTransNode implements GetInputs {
	...
	public ImageIcon getIcon() {
		return ProjectUtils.TRANSFORM_ICON;
	}
	...
	public boolean realEdit(AbstractApatarActions actions) {
		...
		return JProjectDialog.showDialog(prj, this) == JProjectDialog.OK_OPTION;
		...
		try {
			...
			while (rs.next()) {
				try {
					...
					TableInfo ti = getTiForConnection(ProjectNode.OUTPUT_CONN_POINT);
	...
	public List<Record> getInputs() {
		TableInfo ti = getTiForConnection(ProjectNode.INPUT_CONN_POINT);
		if (ti == null) {
			return null;
		}
