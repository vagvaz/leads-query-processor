...
import com.apatar.project.ProjectUtils;
import com.apatar.project.ui.JProjectDialog;
...
public class JProjectDialog extends JDialog {
	...
	public JProjectDialog(final Project project, final ProjectNode node)
			throws HeadlessException {
		super(ApatarUiMain.MAIN_FRAME, "Projection");
		this.node = node;
		inputTI = node.getTiForConnection(ProjectNode.INPUT_CONN_POINT);
		setModal(true);
		...
		try {
			toolBar.AddButton(ProjectUtils.AUTO_MAP, false, this, this
						...
						new JShortcutBar(
								AbstractNode
										.getOtherSideTableInfo(node
												.getConnPoint(ProjectNode.INPUT_CONN_POINT)),
								ProjectNode.INPUT_CONN_POINT,
								node
										.getConnectedNodeName(ProjectNode.INPUT_CONN_POINT),
		...
		outputPanel = new JOutputPanel(node
				...
				ProjectNode.OUTPUT_CONN_POINT, rightSplitPane, node);
		rightSplitPane.setRightComponent(outputPanel);
		//tabbedPane.addTab("Functions", ProjectUtils.SMALL_TRANSFORM_ICON, @@
				//createTabFunction());@@
	...
	public void autoMap() {
		FunctionUtils.createNodeColumns(workPane, project, node
				.getTiForConnection(ProjectNode.OUTPUT_CONN_POINT), node
				.getTiForConnection(ProjectNode.INPUT_CONN_POINT), 390, 10,
				10);
	}
	...
	public void EditOutputs() {
		SchemaTable schema = node.getTiForConnection(
				ProjectNode.OUTPUT_CONN_POINT).getSchemaTable();
	...
	public static int showDialog(final Project project, final ProjectNode node) {
		JProjectDialog dlg = new JProjectDialog(project, node);
