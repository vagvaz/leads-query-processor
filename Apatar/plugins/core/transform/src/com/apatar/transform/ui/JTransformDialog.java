...
import com.apatar.transform.ui.JTransformDialog;
...
public class JTransformDialog extends JDialog {
	...
	public JTransformDialog(final Project project, final TransformNode node)
			throws HeadlessException {
		super(ApatarUiMain.MAIN_FRAME, "Transformation");
		this.node = node;
		inputTI = node.getTiForConnection(TransformNode.INPUT_CONN_POINT);
		setModal(true);
						...
						new JShortcutBar(
								AbstractNode
										.getOtherSideTableInfo(node
												.getConnPoint(TransformNode.INPUT_CONN_POINT)),
								TransformNode.INPUT_CONN_POINT,
								node
										.getConnectedNodeName(TransformNode.INPUT_CONN_POINT),
								false, SwingConstants.LEFT, new String[] {}));

		// merge output schemas
		//node.MergeSchemas(TransformNode.OUTPUT_CONN_POINT); @@GIANNIS_merge

		outputPanel = new JOutputPanel(node
				.getTiForConnection(TransformNode.OUTPUT_CONN_POINT),
				TransformNode.OUTPUT_CONN_POINT, rightSplitPane, node);
		rightSplitPane.setRightComponent(outputPanel);
		tabbedPane.addTab("Functions", TransformUtils.SMALL_TRANSFORM_ICON,
				createTabFunction());
	...
	public void autoMap() {
		FunctionUtils.createNodeColumns(workPane, project, node
				.getTiForConnection(TransformNode.OUTPUT_CONN_POINT), node
				.getTiForConnection(TransformNode.INPUT_CONN_POINT), 390, 10,
				10);
	}
	...
	public void EditOutputs() {
		SchemaTable schema = node.getTiForConnection(
				TransformNode.OUTPUT_CONN_POINT).getSchemaTable();
	...
	public static int showDialog(final Project project, final TransformNode node) {
