...
package com.apatar.mapReduce.ui;
...
public class JTransformDialog extends JDialog {
	...
	public JTransformDialog(final Project project, final MapReduceNode node)
			throws HeadlessException {
		super(ApatarUiMain.MAIN_FRAME, "Map Reduce");
		this.node = node;
		inputTI = node.getTiForConnection(MapReduceNode.INPUT_CONN_POINT);
		setModal(true);
		...
		try {
			toolBar.AddButton(MapReduceNodeUtils.AUTO_MAP, false, this, this
						...
						new JShortcutBar(
								AbstractNode
										.getOtherSideTableInfo(node
												.getConnPoint(MapReduceNode.INPUT_CONN_POINT)),
								MapReduceNode.INPUT_CONN_POINT,
								node
										.getConnectedNodeName(MapReduceNode.INPUT_CONN_POINT),
								false, SwingConstants.LEFT, new String[] {}));

		// merge output schemas
		//node.MergeSchemas(MapReduceNode.OUTPUT_CONN_POINT); @@GIANNIS_merge

		outputPanel = new JOutputPanel(node
				.getTiForConnection(MapReduceNode.OUTPUT_CONN_POINT),
				MapReduceNode.OUTPUT_CONN_POINT, rightSplitPane, node);
		rightSplitPane.setRightComponent(outputPanel);
		tabbedPane.addTab("Functions", MapReduceNodeUtils.SMALL_TRANSFORM_ICON,
				createTabFunction());
	...
	private JShortcutBar createTabFunction() {
		List<NodeFactory> list = FunctionsPlugin.getNodesFunctionMR();
		return new JShortcutBar(list, SwingConstants.LEFT);
	}

	public void autoMap() {
		FunctionUtils.createNodeColumns(workPane, project, node
				.getTiForConnection(MapReduceNode.OUTPUT_CONN_POINT), node
				.getTiForConnection(MapReduceNode.INPUT_CONN_POINT), 390, 10,
				10);
	}
	...
	public void EditOutputs() {
		SchemaTable schema = node.getTiForConnection(
				MapReduceNode.OUTPUT_CONN_POINT).getSchemaTable();
	...
	public static int showDialog(final Project project, final MapReduceNode node) {
