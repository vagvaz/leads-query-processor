...
public class IsIpValidateFunction extends AbstractApatarFunction {
	public Object execute(List list) {
		...
		try {
			if (ApatarRegExp.matchRegExp("(\\d{1,3}\\u002E){3}\\d{1,3}", ip)) {
				return true;
			} else {
				return false;
			}
	...
	static FunctionInfo fi = new FunctionInfo("Is IP Valid", 1, 1);
	static {
