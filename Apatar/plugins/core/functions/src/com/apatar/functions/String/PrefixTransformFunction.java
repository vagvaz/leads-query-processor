...
public class PrefixTransformFunction extends ValueAbstractApatarFunction{

	public Object execute(List list) {
		String delimiter;
		try{
			delimiter = String.valueOf( getValue() );  
		} catch(Exception e ){ return null; }
		
		if( list.get(0) instanceof String)
			return delimiter.concat(list.get(0).toString());
		else
			return list;
			
	}
	
	static FunctionInfo fi = new FunctionInfo("Prefix", 1, 1);
	static 
	{
