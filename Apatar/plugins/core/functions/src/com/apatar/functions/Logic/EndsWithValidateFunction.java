...
public class EndsWithValidateFunction extends ValueAbstractApatarFunction {
	...
	public Object execute(List list) {
		...
		if (((String) ob).length() < value.length()) {
			return false;
		}
		return ((String) ob).lastIndexOf(value) == ((String) ob).length()
				- value.length();
	}

	static FunctionInfo	fi	= new FunctionInfo("Ends With", 1, 1);
	static {
