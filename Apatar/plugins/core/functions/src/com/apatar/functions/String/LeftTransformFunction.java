...
public class LeftTransformFunction extends IntValueAbstractApatarFunction {

	static FunctionInfo fi = new FunctionInfo("Left", 1, 1);
	...
	public Object execute(List list) {
		...
		if( list.get(0) instanceof String ){
			if( list.get(0).toString().length() >= numEndSubString )
				return list.get(0).toString().substring(0, numEndSubString);
			else
				return list.get(0);
		}
		else
