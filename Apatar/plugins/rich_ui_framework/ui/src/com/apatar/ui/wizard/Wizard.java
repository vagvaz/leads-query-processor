...
public class Wizard extends WindowAdapter implements PropertyChangeListener {
	...
	private JCommentPanel commentPanel;

	// private String keyForReferringToDescription;
	 ...
	 */
	public void setCurrentPanel(Object id, String actionCommand) {
		...
		if (oldPanelDescriptor != null) {
			if (oldPanelDescriptor.aboutToHidePanel(actionCommand) == WizardPanelDescriptor.LEAVE_CURRENT_PANEL) {
				return;
			}
		}

		wizardModel.setCurrentPanel(id);
		wizardModel.getCurrentPanelDescriptor().setLastButtonPressed(
				actionCommand);
	...
	void close(int code) {
		...
		if (code == FINISH_RETURN_CODE) {
			if (wizardModel.getCurrentPanelDescriptor().aboutToHidePanel(
					NEXT_BUTTON_ACTION_COMMAND) == WizardPanelDescriptor.LEAVE_CURRENT_PANEL) {
				return;
			}
		}
	...
	private void initComponents() {
		...
		buttonBox.add(Box.createHorizontalStrut(135));
	...
	static {
		...
		try {
			...
			/*
			 * BACK_ICON = new
			 * ImageIcon((URL)getImage((String)(resources.getObject
			 * ("backButtonIcon")))); NEXT_ICON = new
			 * ImageIcon((URL)getImage((String
			 * )(resources.getObject("nextButtonIcon")))); CANCEL_ICON = new
			 * ImageIcon
			 * ((URL)getImage((String)(resources.getObject("cancelButtonIcon"
			 * )))); FINISH_ICON = new
			 * ImageIcon((URL)getImage((String)(resources
			 * .getObject("finishButtonIcon"))));
			 */
	...
	public void setTitleComment(String text) {
		commentPanel.setTitle(text);
	}

	public void setAdditionalComment(String text) {
		commentPanel.setComment(text);
	}

	public void setSize(int width, int heigth) {
		wizardDialog.setSize(width, heigth);
	}
