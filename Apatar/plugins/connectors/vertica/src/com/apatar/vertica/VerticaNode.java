...
public class VerticaNode extends AbstractJdbcDataBase {
	...
	static {
		...
		rcList.add(new DBTypeRecord(ERecordType.Text, "CHAR", 0, 32767, false,
				false));
		rcList.add(new DBTypeRecord(ERecordType.Text, "BPCHAR", 0, 32767,
				false, false));
		...
		rcList.add(new DBTypeRecord(ERecordType.Numeric, "INT", 4, 4, true,
				true));
		rcList.add(new DBTypeRecord(ERecordType.Numeric, "INT8", 4, 4, true,
				true));
		rcList.add(new DBTypeRecord(ERecordType.Numeric, "setInt", 4, 4, true,
				true));
		rcList.add(new DBTypeRecord(ERecordType.Numeric, "BIGINT", 2, 2, true,
				true));
		...
		rcList.add(new DBTypeRecord(ERecordType.Decimal, "FLOAT8", 8, 8, true,
				true));
		rcList.add(new DBTypeRecord(ERecordType.Decimal, "DOUBLE PRECISION", 8,
				8, true, true));
	...
	public ImageIcon getIcon() {
		return VerticaUtils.READ_SYBASE_NODE_ICON;
	}
	...
	public void createDatabaseParam(Wizard wizard) {
		...
		try {
			WizardPanelDescriptor descriptor1 = new DBConnectionDescriptor(
					...
					ApplicationData
							.classForName("com.apatar.vertica.VerticaJdbcParams"),
					"db_connector", "vertica");
			...
			wizard.setKeyForReferringToDescription("help.connector.vertica");
