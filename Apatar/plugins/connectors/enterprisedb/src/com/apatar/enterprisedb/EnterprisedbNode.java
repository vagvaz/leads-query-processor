...
package com.apatar.enterprisedb;
...
public class EnterprisedbNode extends AbstractJdbcDataBase {

	static final DataBaseInfo dataBaseInfo = new DataBaseInfo("\"", "\"", "\"",
			"\"", true, true, true, true, true);

	public EnterprisedbNode() {
		super();
		title = "EnterpriseDB";
	}
	...
	static {
		...
		rcList.add(new DBTypeRecord(ERecordType.Decimal, "float8", 8, 8, true,
				true));
		// rcList.add(new DBTypeRecord(ERecordType.Decimal, "_float8", 8, 8,
		// true, true));
		...
		rcList.add(new DBTypeRecord(ERecordType.Binary, "lseg", 32, 32, false,
				false));
		rcList.add(new DBTypeRecord(ERecordType.Text, "box", 32, 32, false,
				false));
		// rcList.add(new DBTypeRecord(ERecordType.Binary, "_box", 32, 32,
		// false, false));
		...
		rcList.add(new DBTypeRecord(ERecordType.Text, "regoperator", 12, 24,
				false, false));
		// rcList.add(new DBTypeRecord(ERecordType.Text, "regproc", 12, 24,
		// false, false));
	...
	public void createDatabaseParam(Wizard wizard) {
		...
		try {
			WizardPanelDescriptor descriptor1 = new DBConnectionDescriptor(
					...
					ApplicationData
							.classForName("com.apatar.enterprisedb.EnterpriseDBJdbcParams"),
					"db_connector", "enterprisedb");
			...
			wizard
					.setKeyForReferringToDescription("help.connector.enterprisedb");
	...
	public ImageIcon getIcon() {
		return EnterprisedbUtils.READ_ENTERPRISEDB_NODE_ICON;
	}
