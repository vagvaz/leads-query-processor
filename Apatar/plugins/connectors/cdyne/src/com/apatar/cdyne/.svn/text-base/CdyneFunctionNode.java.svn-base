...
public class CdyneFunctionNode extends FunctionNode {

	String licenseKey;
	Cdyne node;

	public CdyneFunctionNode(String classFunc, FunctionInformation fi) {
		super(classFunc, fi);
	}
	...
	public void Transform() throws SocketException {
		...
		setResult(((CdyneFunction) func).execute(values, licenseKey, node));
		DataBaseTools.completeTransfer();
	}

	public void setLicenseKey(String lk) {
		licenseKey = lk;
	}

	public void setOwnerNode(Cdyne node) {
		this.node = node;
	}
